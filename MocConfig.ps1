<#
.description
    이 파일은 사용자 커스터마이징 전용입니다. MOC의 구조나 생성 위치를 바꾸고 싶을 때 이 파일만 수정하면 됩니다.


    
.SYNOPSIS
    경영학 MOC(Map of Content)의 구조와 생성 경로를 정의하는 설정 파일입니다.

.DESCRIPTION
    이 파일은 실행 로직을 담고 있지 않으며, 데이터와 변수 정의만 포함합니다.
    'New-MocFromConfig.ps1' 스크립트가 이 파일을 불러와서 사용합니다.
    MOC 구조를 수정하거나 루트 경로를 변경하려면 이 파일만 수정하세요.
...
#----------------------------------------------------------------------
# ✅ 2. 사용자 루트 경로 설정 (User Configuration)
#----------------------------------------------------------------------
# 생성할 디렉터리와 파일의 최상위 경로를 지정하세요.
$rootPath = "C:\Business_MOC_Project"
#----------------------------------------------------------------------

# 설정 파일의 끝을 알리는 메시지 (선택 사항)
Write-Verbose "MocConfig.ps1: Configuration loaded."